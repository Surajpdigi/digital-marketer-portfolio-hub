    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-auto">
      <div
        ...
        className={`relative bg-white rounded-lg overflow-hidden ${
          isShort ? "w-[350px] max-w-full" : "w-full max-w-4xl"
        } max-h-[90vh]`}
      >
        <Button
          variant="ghost"
        ...
        <div className="p-4 bg-white max-h-[30vh] overflow-y-auto">
          <h3 className="text-xl font-bold mb-2">{currentVideo?.title || "Marketing Video"}</h3>
          <p className="text-muted-foreground">{currentVideo?.description || "Video description"}</p>
        </div>
      </div>
    </div>
  );
